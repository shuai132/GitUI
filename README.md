...
- [x] 工作副本：暂存 / 取消暂存（支持文件级、目录级和 hunk 级及全键盘审阅流程）、提交、amend、discard（未暂存文件、hunk 或全部），提交信息草稿按仓库自动保存与恢复并提供首行长度提示；文件双击在编辑器打开，右键菜单支持复制路径、在 Finder/编辑器/终端中打开、打开 / 初始化 / 更新 Submodule、添加到 .gitignore、按仓库移动到变更列表最前 / 最后；支持文件树状显示
