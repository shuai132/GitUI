...
| --- | --- | --- |
| P0 | 进行中 | 写入总体计划并单独提交 |
| P1 | 待开始 | 优化 `get_status`，避免状态刷新读取未跟踪文件内容 |
| P2 | 待开始 | 细化 watcher 事件类别，减少不必要的 history / tags / stash / submodules 刷新 |
...
- [ ] P0：提交本计划文档。
- [ ] P1：调整后端状态统计策略，让未跟踪文件在状态列表中保持轻量；需要更新 `docs/03-workspace.md`。
- [ ] P2：扩展 watcher 事件 payload，按 worktree / index / refs / config 等类别驱动前端刷新；需要更新 `docs/02-repo-management.md` 和 `docs/11-ipc.md`。
