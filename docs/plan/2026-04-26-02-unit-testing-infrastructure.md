...
| 阶段 | 状态 | 目标 |
| --- | --- | --- |
| 1 | 完成 | 前端测试基建：配置 Vitest 及编写首批算法测试 |
| 2 | 完成 | 后端测试基建：引入 `TestRepo` 并在 Rust 编写 Git 业务测试 |
| 3 | 完成 | CI/CD 集成：编写 GitHub Actions Workflow |

## 子任务清单

- [x] **阶段 1：前端测试基建**
  - [x] `npm i -D vitest @vue/test-utils jsdom`
  - [x] 在 `package.json` 添加 `test` 脚本
  - [x] 编写 `src/utils/graph.spec.ts`，覆盖 `computeGraphLayout` 算法的核心逻辑。
  - [x] 提交 Commit: `build: 增加前端单元测试框架 (Vitest) 及示例用例`

- [x] **阶段 2：后端测试基建**
  - [x] 在 `src/git/` 增加 `test_utils.rs` (包含 `TestRepo` 初始化逻辑，支持生成 mock commit 等)。
  - [x] 在 `src/git/engine.rs` 增加基于 `TestRepo` 的 `get_status` 和 `get_log` 的单元测试。
  - [x] 提交 Commit: `test: 增加 Rust 后端核心 Git 功能的测试基础设施与用例`
