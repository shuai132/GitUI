...
- `list_tags` / `delete_tag` / `list_remote_tags` / `fetch_tags_from_remote` / `delete_remote_tag`：标签管理。返回的 `git/types.rs::TagInfo` 以 `ref_oid` 表示 Tag 引用直接对象，以 `commit_oid` 表示 peel 后的历史 commit。
