...
| `switch_branch` | `repoId, name, force` | `void` |
...
| `push_tag` | `repoId, remoteName, tagName, force` | `void` （refspec `refs/tags/<name>:refs/tags/<name>`；若带 force 则加 `+`） |
| `pull_branch` | `repoId, remoteName, branchName` | `void` |
| `list_remotes` | `repoId` | `string[]` |
| `add_remote` | `repoId, name, url` | `void` |
| `remove_remote` | `repoId, name` | `void`（同时移除所有 remote-tracking refs） |
...
| `deinit_submodule` | `repoId, name` | `void`（完整删除：工作区目录 + .git/modules + .gitmodules + index） |
| `add_submodule` | `repoId, url, path` | `void`（等价于 `git submodule add`，SSH URL fallback 到系统 git） |
...
| `DiffLine` | `DiffLine` |
| `DiffHunk` | `DiffHunk` |
| `FileDiff`（含 `old_blob_oid`, `new_blob_oid` 用于图片预览；`encoding` 为检测到的文件编码名如 `"UTF-8"` / `"GBK"`） | `FileDiff` |
| `BlobData` | `BlobData` |
