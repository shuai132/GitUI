...
- **安全恢复与删除**：侧边栏 Pop 在工作区已有改动时展示唯一改动路径数、Stash 序号和消息；clean 工作区仍保持一步恢复。侧边栏与历史图删除 Stash 都使用应用内确认框说明常规恢复限制。Pop / Drop 携带 stash commit OID，栈在操作期间变化时拒绝按已经重排的 index 处理其他条目；确认期间切换仓库会取消旧请求。
