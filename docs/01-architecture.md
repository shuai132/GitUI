...
| `git_tasks.rs` | Git 阻塞任务的统一调度入口，本地与网络操作分别有界并发。 |
...
4. **阻塞任务隔离**：Git 命令和自动 fetch 在 `git_tasks` 的阻塞线程中打开仓库、执行计算 / 磁盘 / 网络操作。排队先异步等待名额，不占用异步工作线程或空等的阻塞线程；本地与网络名额独立，慢网络不耗尽本地查询容量。具体限额见该模块。不持有 Git 句柄跨 await；命令参数、返回类型与业务错误保持不变，既有写入前上下文校验仍在实际执行时完成。
5. **取消边界**：等待队列中的任务可随调用 future 取消，已开始的同步 Git 操作自然结束，且一直持有并发名额；不宣称中止已经开始的写操作。任务 panic 转换为现有操作失败错误，名额仍释放。
6. **UI 持久化**：用户界面状态（如侧栏宽度、查看模式）与仓库列表分别采用不同的持久化机制。
