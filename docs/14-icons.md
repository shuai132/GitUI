...
- **托盘图标**：macOS 菜单栏使用独立的单色 template 图标源图，透明背景仅保留图形 alpha；运行时通过 Tauri tray template 标记交给系统按浅色 / 深色菜单栏自动着色。Windows / Linux 托盘继续使用应用默认图标。
...
- **平台语义优先**：macOS 菜单栏图标不复用彩色应用图标，避免在暗色菜单栏中保持固定黑色；托盘图标只表达轮廓，颜色交由系统决定。
