...
- 前端：
  ...
  - `components/diff/DiffViewer.vue`（基于 CodeMirror 的只读查看器，保留但当前未被 `HistoryView` 使用）
...
`DiffViewer.vue` 是一个基于 CodeMirror 6 的只读 unified-diff 查看器，当前只被 `CommitDetail.vue` 引用；而 `CommitDetail.vue` 并没有在路由里挂载（已被 `CommitInfoPanel + DiffView` 替代）。保留是为了未来可能的 "集成化 diff 编辑 / 大文件虚拟渲染" 场景，但新开发不再使用它。`package.json` 里的 `@codemirror/*` 依赖都是服务这个模块。
