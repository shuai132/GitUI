...
**非平凡功能 / 重构开工前，先在 `docs/plan/` 下留一份方案文档**，再动代码。小 bugfix、UI 微调、变量改名等不要求。

**文件命名**：`YYYY-MM-DD-NN-name.md`，`NN` 为当日序号（两位数，从 `01` 起，同一天多个方案依次 `01/02/…`），`name` 为 kebab-case 主题。例：`2026-04-17-01-merge-rebase.md`。

**文档必须包含**：

1. **Context**：为什么要做、要解决什么问题、预期结果
2. **进度总览**：按 PR / 阶段拆分的表格，列出范围、状态（`未开始 / 进行中 / 已完成`）、依赖项
3. **子任务清单**：每项前置 `- [ ]` / `- [x]`，便于跟踪
4. **关键决策**：明确做什么、不做什么，取舍理由
5. **验证方式**：如何端到端跑一遍

**勾选状态随实施推进实时更新**——每完成一个子任务就把 `[ ]` 改 `[x]`，状态变化随同主要代码 commit 一起提交。若在实施中发现原方案不合理，先回到 plan 改方案，再继续写代码（与「文档驱动开发」操作规则一致）。

Plan 文档是项目产出的一部分，长期保留在 `docs/plan/` 中，方便回看某次功能演进的决策背景。功能落地后不要删除。
...
**提交信息不要带任何 AI 协作署名**：禁止添加 `Co-Authored-By: Claude ...`、`🤖 Generated with Claude Code` 之类的尾注或标记，保持提交信息只描述本次改动本身。
